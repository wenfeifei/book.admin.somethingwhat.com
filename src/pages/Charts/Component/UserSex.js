...
// 查询列表数据url
const queryDataUrl = 'statistics/getBookUserGenderStatistics';
...
class Donut extends React.Component {
  ...
  render() {
    const {
      loading,
      statistics: { resByBookUserGenderStatistics },
    } = this.props;
    ...
    const data = []
    let sum = 0
    if (resByBookUserGenderStatistics) {
      resByBookUserGenderStatistics.forEach(item => {
        let name = ''
        if (item.flag === '1') name = '男'
        else if (item.flag === '2') name = '女'
        else name = '未知'
        data.push({
          item: name,
          count: item.total,
