...
const CreateForm = Form.create()(props => {
  ...
  const userIdProps = getFieldProps('User_Id', {
    rules: [
      { required: true, min: 4, message: '用户名至少为 4 个字符' },
      // { validator: userExists },
    ],
    initialValue: record ? record.User_Id : '',
  });
  ...
  return (
    ...
    >
      ...
      <FormItem
        ...
        label="用户id"
        hasFeedback
        help={isFieldValidating('User_Id') ? '校验中...' : (getFieldError('User_Id') || []).join(', ')}
      >
  ...
  return (
    ...
    >
      ...
      <FormItem {...formItemLayout} label="用户id">
        <span>{record ? record.User_Id : ''}</span>
      </FormItem>
