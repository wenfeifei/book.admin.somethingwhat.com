...
// 自定义检查响应数据
function checkRemoteData(p) {
  // console.info('自定义检查响应数据', p);
  // const p = response.json();
  p.then(res => {
    // console.info('自定义检查响应数据 res', res, res.Code, res.Msg);
    if (res.Code !== undefined && res.Msg !== undefined) {
      if (res.Code < 0) {
        const errortext = ` 操作失败[${res.Code}]: ${res.Msg}`;
        notification.error({
          message: `服务器返回数据异常`,
          description: errortext,
        });

        if (res.Code === ErrorCode.user_no_authority) {
          // 用户没有权限
        }
        else if (res.Code === ErrorCode.user_not_found) {
          // 用户找不到了
        }
        else if (res.Code === ErrorCode.user_logon_overdue) {
          // 登录凭证已过期，您需要重新登录
          setTimeout(() => {
            router.push('/user/login');
          }, 1000 * 2);
        }
        else if (res.Code === ErrorCode.user_pwd_modified) {
          // 用户密码被修改了
          setTimeout(() => {
            router.push('/user/login');
          }, 1000 * 2);
        }
        else if (res.Code === ErrorCode.user_disabled) {
          // 用户被禁用了
        }
